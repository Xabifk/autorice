...
do
	...
	then
		...
		pacman --noconfirm --needed -S $name >/dev/null 2>&1 || ( echo "[ Could not install $name form Repository ]" )
	fi

	if [ $place = "aur" ]
	then
		echo "[ Installing $name from AUR ]"
		sudo -u "$1" yay -S --noconfirm "$name" >/dev/null 2>&1 || ( echo "[ Could not install $name from AUR ]" )
	fi
